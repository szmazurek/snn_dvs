...
#from models import SNN_2_S as SNN
...
class OneDataset(Dataset):
    def __init__(self, targ_dir: str) -> None:
        ...
        self.all_files = [glob.glob(os.path.join( folders, "*.png")) for folders in self.all_folders]
...
def main():
    ...
    accumulation=10
    checkpoint_file_save = "checkpoint_DVS_EFF_default.pth"
    ...
    datasetRGB = OneDataset(r"/home/plgkrzysjed1/datasets/dataset_dvs")
    train_data_loaderrgb = DataLoader(datasetRGB, batch_size=1, shuffle=False, num_workers=1)
    ...
    net.features[0][0] = nn.Conv2d(1, 24, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)
    ...
    model_coverter = ann2snn.Converter(mode='max', dataloader=train_data_loaderrgb)
    net = model_coverter(net)
    ...
    for epoch in range(0, epochs):
        ...
        for img, label in train_data_loader:
            ...
            y_seq = torch.empty((img.shape[0], 1, 2))
            for t in range(img.shape[0]):
                ...
                l = nn.MSELoss()
                loss = l(output, label_onehot[t].float()) / accumulation
                loss.backward()
                train_loss += loss.item()

                if (t+1) % accumulation == 0:
                    optimizer.step()
                    optimizer.zero_grad()

            train_samples += label.numel()
            #train_loss += loss.item() * label.numel()
            train_acc += (y_seq.argmax(2).to(device) == label).float().sum().item()
            if isinstance(train_f1_list, int):

                train_f1_list = y_seq.argmax(2).cpu()
                train_label_list = label.cpu()
            else:
                train_f1_list = np.vstack((train_f1_list, y_seq.argmax(2).cpu()))
        ...
        with torch.no_grad():
            for img, label in test_data_loader:
                ...
                y_seq = torch.empty((img.shape[0], 1, 2)).to(device)
                for t in range(img.shape[0]):
                    y_seq[t] = net(img[t])  # [T, 1, 2]

                l = nn.MSELoss()
                loss = l(y_seq, label_onehot.float())

                test_samples += label.numel()
                test_loss += loss.item() * label.numel()
                test_acc += (y_seq.argmax(2) == label).float().sum().item()
                if isinstance(test_f1_list, int):
                    test_f1_list = y_seq.argmax(2).cpu()
                    test_label_list = label.cpu()
                else:
                    test_f1_list = np.vstack((test_f1_list, y_seq.argmax(2).cpu()))
