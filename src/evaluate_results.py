...
def extract_model_config_from_run(run_name : str) -> Tuple[str, bool, bool, int]:
    ...
    if any([keyword in run_name for keyword in TEMPORAL_KEYWORD]):
        temporal = True
        n_samples = int(run_name.split("_")[-2])
...
def load_model_from_run(run_name: str,run_id : str, checkpoint_dir : str, device : str = "cuda") -> pl.LightningModule:
    """
...
def sort_losses_paired_with_indexes(losses : List[float], dataset : torch.utils.data.Dataset) -> Tuple[List[float],List[str]]:
    ...
    Returns:
        Tuple[List[float],List[str]]: Tuple of sorted losses and string paths to the data points.
    """
    indexes = list(range(len(losses)))
    indexes.sort(key=lambda x: losses[x], reverse=True)
    sorted_losses = [losses[i] for i in indexes]
    sorted_paths = [dataset.all_files[i] for i in indexes]
    return sorted_losses, sorted_paths
...
def main(args):
    ...
    for run_name, run_id in runs_dict.items():
        ...
        dataloader = DataLoader(dataset, num_workers=8, prefetch_factor=2, batch_size=args.batch_size, shuffle=False)
        model.eval()
        loss_list  = []
        with torch.no_grad():
        ...
        sorted_losses, sorted_paths = sort_losses_paired_with_indexes(loss_list, dataset)
        results = { "sorted_losses" : sorted_losses, "sorted_paths" : sorted_paths}
        save_path = os.path.join(args.results_save_dir, args.project_name, args.group_name, run_name)
        os.makedirs(save_path, exist_ok=True)
        with open(os.path.join(save_path, "sorted_losses_predictions.json"), "w") as file:
