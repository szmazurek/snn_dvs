...
def calculate_pos_weight(labels: List[int]) -> float:
    neg_count, pos_count = torch.unique(torch.tensor(labels), return_counts=True)[1]
...
def construct_datasets(
    ...
    if dataset_type == "single_sample":
        ...
        train_dataset = dataset_class(
            ...
            sample_len=parameters["dataset"]["timestep"],
            overlap=parameters["dataset"]["overlap"],
            n_frames_predictive_horizon=parameters["dataset"][
...
def train_normal_loop(parameters: dict):
    ...
    DATASET_SAVE_DIR = (
        parameters["dataset_save_dir"]
        if "dataset_save_dir" in parameters.keys()
        else None
    )
    RESTORE_CHECKPOINT_PATH = (
        parameters["checkpoint_restore_path"]
        if "checkpoint_restore_path" in parameters.keys()
        else None
    )
    # model params
    ...
    if DATASET_TYPE == "repeated":
        WANDB_NAME = f"{MODEL_NAME}_{DATASET_TYPE}_{REPEATS}_{DATASET_SUBSET}_{MODALITY}_{IMG_SIZE[0]}x{IMG_SIZE[1]}"
    elif DATASET_TYPE == "temporal":
        WANDB_NAME = f"{MODEL_NAME}_{DATASET_TYPE}_{MODALITY}_{DATASET_SUBSET}_{IMG_SIZE[0]}x{IMG_SIZE[1]}_{TIMESTEP}_{OVERLAP}"
    elif DATASET_TYPE == "prediction_temporal":
        WANDB_NAME = f"{MODEL_NAME}_{DATASET_TYPE}_{MODALITY}_{DATASET_SUBSET}_{IMG_SIZE[0]}x{IMG_SIZE[1]}_{TIMESTEP}_{OVERLAP}_horizon_{N_FRAMES_PREDICTIVE_HORIZON}"
    elif DATASET_TYPE == "prediction_repeated":
        WANDB_NAME = f"{MODEL_NAME}_{DATASET_TYPE}_{REPEATS}_{MODALITY}_{DATASET_SUBSET}_{IMG_SIZE[0]}x{IMG_SIZE[1]}_horizon_{N_FRAMES_PREDICTIVE_HORIZON}"
    elif DATASET_TYPE == "prediction_single_sample":
        WANDB_NAME = f"{MODEL_NAME}_{DATASET_TYPE}_{MODALITY}_{DATASET_SUBSET}_{IMG_SIZE[0]}x{IMG_SIZE[1]}_horizon_{N_FRAMES_PREDICTIVE_HORIZON}"
    else:
        WANDB_NAME = f"{MODEL_NAME}_{DATASET_TYPE}_{MODALITY}_{DATASET_SUBSET}_{IMG_SIZE[0]}x{IMG_SIZE[1]}"
    video_list = [
        os.path.join(DATA_ROOT_PATH, video) for video in os.listdir(DATA_ROOT_PATH)
    ]
    ...
    if int(os.environ["SLURM_PROCID"]) == 0:
        ...
        trainer.test(
            model=model, dataloaders=test_loader, ckpt_path=best_checkpoint_path
        )
        wandb.finish()
...
if __name__ == "__main__":
    ...
    if parameters["dataset"]["evaluation_mode"] == "kfold":
        raise NotImplementedError("Kfold not implemented yet")
    else:
