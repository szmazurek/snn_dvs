...
class PredictionDvsDataset(Dataset):
    ...
    def load_image(self, image) -> Image.Image:
        ...
        if self.flip_random==1:
            return self.transform_flip(img)
        else:
    ...
    def __getitem__(self, index: int) -> Tuple[torch.Tensor, int]:
        self.flip_random = random.randint(0, 1)
        ...
        images_tensor = binarize(images_tensor)
...
def binarize(img):
    return torch.where(img>0,torch.ones_like(img), torch.zeros_like(img))
...
# if __name__ == "__main__":
#     wandb.agent("krzysjed/SpikingPred/u9dy3z29", function=main, count=500)